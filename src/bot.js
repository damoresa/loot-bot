    ...
    client.on('message', async (message) => {
        if (message.author.username !== CONSTANTS.BOT_NAME && message.content.startsWith('!')) {
            if (CONSTANTS.COMMANDS_REGEXP.HELP.test(message.content)) {
                ...
                response +=
                    ' - !loot LOOTPASTE : records the given loot data on the database. A reply with the hunt code is generated aswell.\n';
                response +=
                    ' - !expense HUNTCODE AMOUNT PINCODE : records the given supplies expense to the hunt with the given code and pin code. Your Discord user is used to keep track of who reported the expense.\n';
                response +=
                    ' - !balance HUNTCODE : displays the money balance for the given hunt code aswell as who reported expenses.\n';
                response +=
                    ' - !monthxp : displays the amount of experience obtained by the user executing the command.\n';
                response += ' - !monthhunts : displays the recorded hunts during this month with participants.\n';
                response +=
                    ' - !monthbalance : displays the recorded balance generated by combining all your hunts during this month.\n';
                response += '\n';
                ...
                try {
                    const lootReport = await Parser.parseLoot(
                    ...
                    if (output.code) {
                        response += '\n';
                        response += `This hunt has been given the code ${
                            output.code
                        }. If you wish to add expenses to it, use the !expense command.`;
                    }

                    const reporterMessage = `The pincode for your hunt ${output.code} is ${
                        output.pinCode
                ...
                try {
                    const expenseReport = await Parser.parseExpense(
                        message.author.username,
                        message.author.id,
                        message.content
                    );
                    ...
                    const response = `Expense of ${output.amount} registered for hunt ${output.code} by ${
                        output.reporter
                    }.`;
                    message.reply(response);
                } catch (err) {
                    ...
                    message.reply(
                        "Unable to store expense, please remember to input the correct pin code.\nIf you don't have one, ask the hunt reporter for it."
                    );
                }
                ...
                try {
                    ...
                    for (const share of balance.balances) {
                        response += ` - ${share.reporter}: ${share.balance} (${share.balance -
                ...
                try {
                    ...
                    if (output === 0) {
                        ...
                        response +=
                            'NOTE: Keep in mind deaths are not being tracked, so experience losses are not accounted.';
                    }

                    message.reply(response);
                } catch (err) {
                    winston.error(
                        `Unable to generate monthly experience report for user ${message.author.username}: ${err}`
                    );
                ...
                try {
                    ...
                    winston.error(
                        `Unable to generate this month's hunts report for user ${message.author.username}: ${err}`
                    );
                ...
                try {
                    ...
                    if (output.expenses === 0 && output.loot === 0) {
                        ...
                        response = `You've spent ${output.expenses} supplies and your hunts have generated ${
                            output.loot
                        } split loot.`;
                    }
                    ...
                    winston.error(
                        `Unable to generate this month's hunts balance report for user ${
                            message.author.username
                        }: ${err}`
                    );
                ...
                try {
                    const paymentReport = await Parser.parsePayment(
                        message.author.username,
                        message.author.id,
                        message.content
                    );
                ...
                message.reply(
                    'Unknown or wrong frommated command, please try !help to learn about the available commands.'
                );
            }
        }
    });
